...
지금 햇갈리는 부분은 반복되는 뷰 로직을 적절하게 처리하지 못했는데 이걸 Array를 사용해서 정규화를 해서 반복을 줄였어야 했나 싶다.
...
이 부분이 똑같이 반복되기 때문에 FormItemContainer를 컴포넌트로 따로 격리하고 반복을 줄였다면 어땠을까?
예를 들자면 이렇다.
...
하지만 이렇게 하면 FormItemContainer는 변경점이 있을 때마다 많은 고민을 해야한다. 유연함이 없어지기 때문에 예외 상황이 생겼을 때 props를 하나 더 받아야하고
props에 따라 FormItemContainer를 분기처리해야하는 상황이 생긴다. 그래서 고민하다 그냥 FormItemContainer라는 컴포넌트를 만들기보다.
...
이번에 Join.tsx를 리펙토링 하면서 방향을 잡기가 쉽지는 않았다. 하지만 일단 코드를 읽고 코드의 동작 방법을 적어보고 큰 부분으로 나누고 다시 코드를 작성하고를 반복하면서
