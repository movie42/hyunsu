...
export function generateStaticParams() {
	return getPostsByCategory('programming').map((post) => ({ slug: [post.slug] }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
	const { slug: slugParts } = await params;
	const slug = slugParts.join('/');
	const post = getPostByCategoryAndSlug('programming', slug);
	if (!post) return {};
	...
	return {
		...
		alternates: {
			canonical: `/posts/programming/${post.slug}`
		},
		openGraph: {
			type: 'article',
			title: post.title,
			description: post.description,
			url: `/posts/programming/${post.slug}`,
...
export default async function ProgrammingPostPage({ params }: PageProps) {
	const { slug: slugParts } = await params;
	const slug = slugParts.join('/');
	const post = getPostByCategoryAndSlug('programming', slug);
