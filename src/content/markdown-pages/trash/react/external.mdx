...
const TodoListItem = ({ isCreator, isWritePermission, todoItem, userId }) => {
  const isDisabled =
    (!isCreator && !isWritePermission) || todoItem.creator !== userId;
  ...
  return (
    <TodoContainer>
      <TodoRow>
        ...
        <EditButton onClick={handleEdit} disabled={isDisabled}>
          수정
        </EditButton>
        <DeleteButton onClick={handleDelete} disabled={isDisabled}>
          삭제
        </DeleteButton>
      </TodoRow>
    </TodoContainer>
...
const TodoListItem = ({ isCreator, isWritePermission, todoItem, userId }) => {
  const isDisabled =
    !!todoItem?.external ||
    (!isCreator && !isWritePermission) ||
    todoItem.creator !== userId;
  ...
  return (
    <TodoContainer>
      <TodoRow>
        ...
        {todoItem?.external ? (
          <LikeButton onClick={handleLikeButton}>{todoItem?.like}</LikeButton>
        ) : null}
      </TodoRow>
    </TodoContainer>
  );
...
> "외부 연동이 아마 더 많아 질겁니다. 외부 연동을 관리할 수 있는 방안을 생각해보세요."
